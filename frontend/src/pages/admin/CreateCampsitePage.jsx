  ...
  useEffect(() => {
    const loadPage = async () => {
      ...
      try {
        ...
        console.error(err);
        setError("Impossible de charger la page de création du site.");
      } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        maxEquipmentLengthFeet:
          form.maxEquipmentLengthFeet === "" ? null : Number(form.maxEquipmentLengthFeet),
      ...
      await api.post("/campsites", payload);

      setSuccessMessage("Site créé avec succès.");

      setTimeout(() => {
        navigate(`/site-manager/campgrounds/${campgroundId}/sites`, { replace: true });
      }, 800);
    } catch (err) {
      console.error(err);
      setError(err.message || "Impossible de créer le site.");
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="min-h-screen bg-slate-50">
      <div className="max-w-6xl mx-auto px-6 py-10">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <Section title="Indisponibilités temporaires">
            <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4">
              <div className="flex items-start gap-3">
                <AlertTriangle className="h-5 w-5 text-amber-600 mt-0.5 shrink-0" />
                <div>
                  <p className="text-sm font-semibold text-amber-900">
                    Cette section sera disponible après la création du site
                  </p>
                  <p className="mt-1 text-sm text-amber-800">
                    Une fois le site créé, tu pourras ajouter des périodes
                    d’indisponibilité temporaires.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            >
              ...
              {submitting ? "Enregistrement..." : "Créer le site"}
            </button>
          </div>
        </form>
      </div>
